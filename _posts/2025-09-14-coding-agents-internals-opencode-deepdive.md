...
## Intro
Coding agents are the killer application of AI agents. You read an article or watch a video explaining AI agents (an LLM in a loop with tools) and think: that's nothing groundbreaking. But oh boy—was I wrong.

The building blocks might seem approachable (LLM APIs, tool calls, prompts, etc.), but making them all work together reliably while offering a smooth developer experience is quite a feat.

Opencode is a fantastic project that is a 100% open-source Claude Code alternative, with a focus on TUI and a client/server architecture. This means you can drive the agent remotely—say, from a mobile app. You can even share sessions as a bonus (so others can follow along with your conversation or session).
...
## Architecture
![alt text](assets/opencode-arch.png)
